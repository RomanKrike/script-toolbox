...
class DisplayField(QtGui.QLineEdit):
    ...
    def refresh(self):
        self.setText(
            self.toolbox.field_display_text(self.item_id)
        )
        if not self.selectable:
            self.deselect()

    def selected_values(self):
        return self.toolbox.field_display_values(self.item_id)
    ...
    def mouseMoveEvent(self, event):
        if self.selectable:
            QtGui.QLineEdit.mouseMoveEvent(self, event)
        else:
            event.accept()

    def mouseDoubleClickEvent(self, event):
        if self.select_scene:
            self.toolbox.select_field_objects(self.item_id)

        if self.selectable:
            QtGui.QLineEdit.mouseDoubleClickEvent(self, event)
        else:
            event.accept()
...
class DisplayFieldList(QtGui.QListWidget):
    ...
    def __init__(
        ...
        height = self.visible_rows * row_height + 6
        self.setMinimumHeight(height)
        self.setMaximumHeight(height)

    def refresh(self):
        selected = set(self.selected_values())
        values = self.toolbox.field_display_values(self.item_id)

        self.blockSignals(True)
        try:
            self.clear()
            for value in values:
                entry = QtGui.QListWidgetItem(text_type(value))
                entry.setToolTip(text_type(value))
                self.addItem(entry)
    ...
    def keyPressEvent(self, event):
        try:
            is_copy = event.matches(QtGui.QKeySequence.Copy)
        except Exception:
            is_copy = False
...
class RuntimeFolder(QtGui.QFrame):
    ...
    def __init__(
        ...
        if not self.embedded:
            ...
            if self.folder_type == "collapsible":
                ...
                self.header_button.setObjectName("RuntimeFolderHeader")
                self.header_button.setSizePolicy(
                    QtGui.QSizePolicy.Expanding,
                    QtGui.QSizePolicy.Preferred
                )
                self.header_button.setFocusPolicy(QtCore.Qt.NoFocus)
    ...
    def update_state(self):
        if self.embedded or self.folder_type != "collapsible":
        ...
        if self.header_button is not None:
            marker = u"\u25b8" if collapsed else u"\u25be"
            self.header_button.setText(
                u"{0}  {1}".format(marker, self.header_label)
            )
            self.header_button.setProperty("collapsed", collapsed)
